...
public final class Members {
	...
	public Members nonSecretaries() {
		List<Member> members = values.stream().filter(Member::nonSecretary).collect(Collectors.toList());
		return new Members(members);
	}
	 ...
	 * @return
	 */
	public Members add(@Nonnull Member member) {
		//TODO:Q.下記バリデーションはaddメソッドで行うべきか、ファクトリメソッド(コンストラクタ)で行うべきか
		if (this.values.stream().anyMatch(v -> v.getName().getValue().equals(member.getName().getValue()))) {
			throw new IllegalArgumentException("すでに同じ名前が登録されています。");
		}
		return of(member, this.values);
	}
	 ...
	 * @return
	 */
	public Members delete(@Nonnull MemberName memberName) {
		List<Member> members = this.values.stream()
				.filter(member -> !member.getName().getValue().equals(memberName.getValue()))
